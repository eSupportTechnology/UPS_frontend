...
const TechnicianTicketModal: React.FC<TechnicianTicketModalProps> = ({ open, onClose, ticket }) => {
    ...
    const handleInventoryUsage = async (usages: InventoryUsageItem[], notes: string) => {
        ...
        try {
            ...
            if (response.success) {
                ...
                throw new Error(response.message || 'Failed to accept ticket');
            }
        } catch (error: any) {
            showAlert({
                type: 'error',
    ...
    const handleInventoryReturn = async (returns: InventoryUsageItem[], notes: string) => {
        ...
        try {
            ...
            if (response.success) {
                ...
                if (fullTicketData) {
                    setFullTicketData({ ...fullTicketData, status: 'completed' });
                }
                ...
                throw new Error(response.message || 'Failed to complete ticket');
            }
        } catch (error: any) {
            showAlert({
                type: 'error',
    ...
    const canComplete = displayTicket?.status === 'accepted' || displayTicket?.status === 'in-progress';
...
export default TechnicianTicketModal;
