...
export const useAlert = () => {
    ...
    const showAlert = (alertProps: AlertProps) => {
        ...
        const newAlert: AlertState = {
            ...
            id,
            visible: true,
            duration: alertProps.duration || 5000,
        };
    ...
    const AlertContainer = () => <AlertManager alerts={alerts} removeAlert={removeAlert} />;
...
const AlertComponent: React.FC<AlertComponentProps> = ({ alert, onClose }) => {
    ...
    const getAlertStyles = () => {
        const baseStyles = `
      transform transition-all duration-300 ease-in-out min-w-[350px] max-w-[450px]
      border-l-4 rounded-md shadow-lg p-4 ${isVisible ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0'}
    `;
